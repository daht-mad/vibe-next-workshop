...
## Workshop Q&A Examples

실제 워크샵에서 진행된 인터뷰 예시입니다. 이 흐름을 참고하여 참석자와 대화하세요.

### 예시: 금융 크리에이터 리드 수집 자동화

**Q1. 지금 회사에서 어떤 일을 맡고 계세요?**
> A1. 유저그로스 리드를 맡고 있어요.

**Q2. 주로 어떤 일을 하시나요? 하루가 어떻게 흘러가나요?**
> A2. 주로 아직은 프로덕트 런칭 전이라 LMF 테스트를 돌리고 있어요. 금융 콘텐츠 서비스를 만들고 있어서 CBT 앱을 런칭해서 계속 앱 테스트를 진행하고 유저 성장을 위한 프로덕트 기획 개발도 같이 참여하고 있어요.

**Q3. 가장 반복적이고 자동화하고 싶은 업무가 뭐예요?**
> A3. 유튜브, 틱톡, 릴스에 있는 크리에이터들의 리스트를 업데이트하는 일이요. 현재 creator-admin-tool 사이트에 금융 크리에이터 리드를 확보하고 있는데, 이게 가장 반복적이고 전문성이 떨어지다 보니 자동화해주면 좋겠어요.

**Q4. 크리에이터는 어떻게 찾고 계세요?**
> A4. 여러 가지 방법이 있는데, 일단 #stockmarket #crypto #investment 등의 해시태그를 기반으로 찾거나, 찾은 크리에이터와 연관성이 높은 크리에이터를 파생해서 찾아가고 있어요.

**Q5. 크리에이터 정보로 어떤 것들을 수집하세요?**
> A5. handle, URL, contact info (email), Platform (YouTube/TikTok/Instagram), Content type (UGC/PGC/AIGC/Publisher), Vertical1, Vertical2, Country, Language, Followers 등이요.

**Q6. 수집한 정보는 어디에 정리하세요?**
> A6. bulk upload용 스프레드시트를 다운받고, 정보를 넣고 Bulk로 업로드해요.

**Q7. 손으로 하면 얼마나 걸려요?**
> A7. 총 10명 정도밖에 못하고... 하루 정도 걸려요.

**Q8. Content type 분류는 어떤 기준으로 하세요?**
> A8. 이미 있는 자료를 바탕으로 해주면 좋을 것 같아요. (기존 CSV 파일의 패턴 참고)
...
| 항목 | 내용 |
|-----|------|
| **스킬명** | finance-creator-finder |
| **트리거** | "금융 크리에이터 찾아줘", "크리에이터 리스트 업데이트" |
| **입력** | 검색 키워드 (해시태그), 플랫폼 선택, 수집할 인원 수 |
| **출력** | CSV 파일 (템플릿 형식에 맞춤) |
| **외부 연동** | YouTube Data API, 브라우저 자동화 (TikTok/Instagram) |
| **복잡도** | 중간 (API 키 + 브라우저 자동화) |

---

### 인터뷰 팁

1. **구체적인 숫자 물어보기**: "몇 명?", "얼마나 걸려요?"
2. **현재 방식 파악하기**: "지금은 어떻게 하고 계세요?"
3. **결과물 목적지 확인**: "어디에 저장하거나 공유하세요?"
4. **기존 자료 활용**: "참고할 수 있는 예시 파일 있으세요?"
5. **Pain Point 공감**: "아 그거 진짜 귀찮죠! 매번 수작업이면 힘들겠어요."
